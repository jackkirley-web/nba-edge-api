...
# Falls back gracefully at every step so the app always has data to show
...
class MainAFLCache:
    ...
    def _fetch_all(self) -> dict:
        from afl_data import (
            ...
            get_team_news, get_synthetic_player_pool, TEAM_ABBREV,
        )
        ...
        # Round fixtures (with multi-source fallback)
        ...
        if not games:
            logger.warning("No AFL games found even after fallbacks")
            return {
        ...
        # Player season averages -- try Footywire, fall back to synthetic
        logger.info("Fetching AFL player averages...")
        player_avgs = get_player_season_averages(year)
        if not player_avgs:
            logger.warning("Footywire unavailable -- using synthetic player pool")
            player_avgs = get_synthetic_player_pool(games)
        ...
        for game in games:
            ...
            try:
                ...
                # Use real odds if available, otherwise use a typical AFL line
                ...
                if not total_line:
                    # Use venue average as estimated total
                    total_line = venue.get("avg_total", 157)
            ...
            for is_home, team_name in [(True, home), (False, away)]:
                ...
                for pname, pdata in sorted_players:
                    ...
                    try:
                        # Build synthetic logs from season averages
        ...
        for p in sorted(
            ...
            top_prop_legs.append({
                ...
                "confidence":p["confidence"], "prob": p["prob"],
...
def _synthetic_logs(pdata: dict, n: int = 12) -> list:
    """Build synthetic game logs from season averages for props projection."""
    import random
    logs = []
    base = {
...
class AFLStreakCache:
    ...
    def _background_fetch(self):
        try:
            ...
            if not player_avgs:
                logger.warning("AFL streak: no player data")
                with self._lock:
                    self._loading = False
                return

            # Build synthetic logs for streak calculation
            player_logs = {}
