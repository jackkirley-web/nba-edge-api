# cache.py — Full pipeline including streaks
...
class NBACache:
    ...
    def _fetch_all(self) -> dict:
        ...
        logger.info("Fetching logs for %d players...", len(players_to_fetch))
        ...
        for game in games:
            ...
            try:
                ...
                if game_odds.get("spread_line") is not None:
                    ...
                    all_legs.append({**sr,"game_id":game["game_id"],"game":away_abbrev+" @ "+home_abbrev,"selection":sel,"odds":game_odds.get("spread_odds",1.91)})
                ...
                if game_odds.get("total_line") is not None:
                    ...
                    all_legs.append({**tr,"game_id":game["game_id"],"game":away_abbrev+" @ "+home_abbrev,"selection":tr["selection_direction"]+" "+str(tl),"odds":game_odds.get("total_odds",1.91)})
                ...
                for is_home, team_id, team_name, team_abbrev_local in [
                    ...
                    for pid in team_pids[:10]:
                        ...
                        player_dict = {**pdata,"usage_rate":adv_match.get("usage_rate",0.15),"minutes":pdata.get("mins",20.0),"position":pdata.get("position","G")}
                        try:
                            prop_result = project_player_props(
                                player=player_dict, game_logs=logs,
                                opp_advanced=opp_ctx["advanced"],
                                home_ctx=home_ctx, away_ctx=away_ctx,
                                player_is_home=is_home, injury_status=inj_status,
                                teammate_injuries=team_inj,
                            )
                            if prop_result and prop_result["scored_props"]:
                                for prop in prop_result["scored_props"]:
                                    all_props.append({**prop,"game_id":game["game_id"],"game":away_abbrev+" @ "+home_abbrev,"team":team_abbrev_local,"player_id":pid,"is_bench":pdata.get("mins",0)<28})
                        except Exception as e:
        ...
        try:
            ...
            logger.info("Streaks calculated: %d", len(streaks))
        except Exception as e:
            logger.error("Streak calculation failed: %s", e)
            streaks = []

        # ── Build multis ──────────────────────────────────────
        top_props = sorted(
            [p for p in all_props if p.get("confidence",0)>=62 and p.get("est_line") is not None],
        ...
        for p in top_props:
            ...
            prop_legs.append({
                "game_id":p["game_id"],"game":p["game"],"type":"Prop — "+p["stat_label"],
                "selection":sel,"odds":1.91,"confidence":p["confidence"],"prob":p["prob"],
...
def _build_context(team_id, abbrev, full_name, is_home,
    ...
    return {
        "team_id":team_id,"team_abbrev":abbrev,"team_name":full_name,"is_home":is_home,
        "advanced":advanced,"recent_l5":recent_l5,"recent_l10":recent_l10,
        "game_logs":[],"rest":rest,"splits":splits,"players":players,
