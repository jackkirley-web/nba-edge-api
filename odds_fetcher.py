# odds_fetcher.py — Robust team matching regardless of home/away order
...
def fetch_odds_for_games(games: list) -> dict:
    try:
        ...
        if not isinstance(odds_data, list):
            logger.error("Odds API unexpected response type")
            return {}
    ...
    # Log available matchups
    for e in odds_data:
        away = e.get("away_team", "")
        home = e.get("home_team", "")
        logger.info("Odds available: %s @ %s", away, home)

    # Build lookup by frozenset of ABBREVIATIONS (order-independent)
    odds_lookup = {}
    ...
    logger.info("Built odds lookup with %d matchups", len(odds_lookup))
    ...
    for game in games:
        ...
        if not match:
            logger.warning("No odds match for %s @ %s", nba_away, nba_home)
            continue
    ...
    logger.info("Odds matched %d of %d games", len(result), len(games))
    return result


def _parse_event(event: dict, odds_home_full: str, odds_home_abbrev: str, nba_home_abbrev: str) -> dict:
    """
    Parse odds from a matched event.
    odds_home_full: the team Odds API considers home
    nba_home_abbrev: the team NBA Live considers home
    These may differ — we adjust spread sign accordingly.
    """
